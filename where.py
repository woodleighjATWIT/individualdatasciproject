...
all_crime_by_street_over_time_2 = all_crime_by_street_over_time.reset_index().drop(columns=['UCR_PART']).groupby(['STREET', 'DISTRICT'])['FREQUENCY'].sum()
...
fig3.subplots_adjust(hspace=.4, wspace=-.8)

p1 = sns.barplot(ax=ax3, x='STREET', y='FREQUENCY', dodge=False, data=all_crime_by_street_over_time)
p1.set(xlabel='Date (Month)', ylabel='Number of Crimes', title='All Crimes by District')
ax3.tick_params(axis='x', rotation=45)
ax3.legend(bbox_to_anchor=(1.18, 1), loc='upper right', borderaxespad=0)
fig3.subplots_adjust(right=0.84, left=.05)
